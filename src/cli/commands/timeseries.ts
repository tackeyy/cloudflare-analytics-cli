...
import { formatTimeseries } from "../../lib/formatter.js";
import type { OutputMode } from "../../lib/types.js";

export function registerTimeseriesCommand(
  program: Command,
  getOutputMode: () => OutputMode,
): void {
  program
    .command("timeseries")
    .description("Show daily page views timeseries")
    ...
    .action(async (opts) => {
      try {
        ...
        const points = await client.getTimeseries({
          siteTag: config.siteTag!,
          dateRange: { from: opts.from, to: opts.to },
          limit: 366,
        });
        console.log(formatTimeseries(points, getOutputMode()));
      } catch (err: any) {
